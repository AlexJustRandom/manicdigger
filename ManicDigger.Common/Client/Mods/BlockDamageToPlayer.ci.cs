public class ModBlockDamageToPlayer : ClientMod
{
	...
	internal void UpdateBlockDamageToPlayer(Game game, float dt)
	{
		...
		int damage = game.blocktypes[block1].DamageToPlayer + game.blocktypes[block2].DamageToPlayer;
		if (damage > 0)
		{
			int hurtingBlock = block1;  //Use block at eyeheight as source block
			if (hurtingBlock == 0 || game.blocktypes[hurtingBlock].DamageToPlayer == 0) { hurtingBlock = block2; }    //Fallback to block at feet if eyeheight block is air or doesn't deal damage
